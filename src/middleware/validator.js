  ...
  switch (field) {
    case "startDate": {
      return check(field).custom(async (value, { req }) => {
        const now = dayjs();
        const startDate = value ? dayjs(value, "YYYY-MM-DD HH:mm:ssZ") : null;
    ...
    case "endDate": {
      return check(field).custom(async (value, { req }) => {
        ...
        const endDate = value ? dayjs(value, "YYYY-MM-DD HH:mm:ssZ") : null;
    ...
    case "confirmPassword": {
      return check(field).custom((confirmPassword, { req }) => {
        if (!confirmPassword) {
          ...
          throw new Error(translate("validations", "password.mismatch"));
        }
